 ...
 */
package com.rhnxdev.hzplayer.data.datasource.player.mp4fork;

import androidx.media3.extractor.mp4.PsshAtomUtil;
import androidx.media3.extractor.mp4.SmtaAtomUtil;
...
import com.rhnxdev.hzplayer.data.datasource.player.ffmpeg.FfmpegMimeTypes;
...
public final class BoxParser {
  ...
  // HZPLAYER-PATCH: QuickTime pro-codec sample entries Media3 drops. Mapped to
  // custom MIME types decoded by the FFmpeg software renderers.
  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_apch = 0x61706368; // ProRes 422 HQ

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_apcn = 0x6170636e; // ProRes 422

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_apcs = 0x61706373; // ProRes 422 LT

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_apco = 0x6170636f; // ProRes 422 Proxy

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_ap4h = 0x61703468; // ProRes 4444

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_ap4x = 0x61703478; // ProRes 4444 XQ

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_AVdn = 0x4156646e; // DNxHD

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_jpeg = 0x6a706567; // Photo-JPEG

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_mjpa = 0x6d6a7061; // Motion-JPEG A

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_mjpb = 0x6d6a7062; // Motion-JPEG B

  @SuppressWarnings("ConstantCaseForConstants")
  private static final int TYPE_CFHD = 0x43464844; // GoPro CineForm

  /**
   * Maps a QuickTime pro-codec sample entry fourcc to the sample MIME type decoded by the FFmpeg
   * software renderers, or {@code null} if {@code atomType} is not one of the patched-in codecs.
   */
  @Nullable
  public static String proCodecMimeTypeForAtom(int atomType) {
    if (atomType == TYPE_apch
        || atomType == TYPE_apcn
        || atomType == TYPE_apcs
        || atomType == TYPE_apco
        || atomType == TYPE_ap4h
        || atomType == TYPE_ap4x) {
      return FfmpegMimeTypes.VIDEO_PRORES;
    } else if (atomType == TYPE_AVdn) {
      return FfmpegMimeTypes.VIDEO_DNXHD;
    } else if (atomType == TYPE_jpeg || atomType == TYPE_mjpa || atomType == TYPE_mjpb) {
      return MimeTypes.VIDEO_MJPEG;
    } else if (atomType == TYPE_CFHD) {
      return FfmpegMimeTypes.VIDEO_CINEFORM;
    }
    return null;
  }
  // HZPLAYER-PATCH-END
  ...
  private static StsdData parseStsd(
    ...
    for (int i = 0; i < numberOfEntries; i++) {
      ...
      if (childAtomType == Mp4Box.TYPE_avc1
          ...
          // HZPLAYER-PATCH: QuickTime pro codecs handed to the FFmpeg renderers.
          || proCodecMimeTypeForAtom(childAtomType) != null) {
  ...
  private static void parseVideoSampleEntry(
    ...
    } else if (proCodecMimeTypeForAtom(atomType) != null) {
      // HZPLAYER-PATCH: intra-frame pro codecs; no config child box needed, the
      // FFmpeg decoder reads everything from the bitstream.
      mimeType = proCodecMimeTypeForAtom(atomType);
    }
      ...
      if (childAtomType == Mp4Box.TYPE_avcC) {
        ...
        if (vexuData != null && vexuData.eyesData != null) {
          if (vpsData != null && vpsData.layerInfos.size() >= 2) {
            ...
            // Based on subsection 1.4.3 of Appleâ€™s proposed ISOBMFF extensions for stereo video
